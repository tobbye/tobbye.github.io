...
function __Land() {
    ...
    this.creatBlock = function(content, data) {
        ...
        if (data.isResult) {
            data.viceStr = data.vice.replace('#len', data.lines.length);
...
function __Depot() {
    ...
    this.toOccupt = function() {
        let pos = Parse.cutZero(this.tgtPos.replace('000000', 'home'));
        let flex = Alert.curFlex;
        ...
        if (lines.length < lines[0].ladd) {
            ...
            line.pos = pos + '-' + ~~(Math.random()*100+1);
            lines.unshift(line);
            Alert.creatContent(Land, 0, 0);
